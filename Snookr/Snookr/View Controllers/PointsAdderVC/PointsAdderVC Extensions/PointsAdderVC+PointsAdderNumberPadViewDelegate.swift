...
extension PointsAdderVC: PointsAdderNumberPadViewDelegate {
    func didTapNumberPad(buttonTag: Int) {
        ...
        case 1, 2, 3, 4, 5, 6, 7, 8, 9, 0:
            ...
            if numberPadView.keyDelete.isHidden { numberPadView.keyDelete.isHidden = false }
            if !addPointsButton.isEnabled { addPointsButton.enableSolidStyleButton() }
            
        case -1: //"-1" is tag for delete key
            pointsString.removeLast()
            if pointsString.isEmpty {
                numberPadView.keyDelete.isHidden = true
